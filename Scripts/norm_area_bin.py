...
def return_plot_series_mean(df):
    temp = df
    temp['NormArea'] = temp['Area_2D']/temp['Max_Area']
    tmp_pvt = pd.pivot_table(temp, values=['NormArea'], index=['TripDate'], aggfunc=np.std)
    tmp_pvt = tmp_pvt.rename(columns={'NormArea':'std_dev'})
    tmp_count = pd.pivot_table(temp,values=['NormArea'], index=['TripDate'], aggfunc='count')
    tmp_count = tmp_count.rename(columns={'NormArea':'count'})
    tmp_pvt['std_error'] = tmp_pvt['std_dev']/np.sqrt(tmp_count['count'])
    tmp_pvt = tmp_pvt[['std_error']]    
    table1 = df
    table1['NormArea']= table1['Area_2D']/table1['Max_Area']
    table1 = pd.pivot_table(table1, values=['NormArea'], index=['TripDate'], aggfunc=np.mean)
    table1 = table1[['NormArea']]
    table1 = table1.merge(tmp_pvt, left_index=True, right_index=True, how='left')
    return table1
...
subset = data[(data.Time_Series == 'long') & (data.Site !='m006r')& (data.Site !='033l') & 
              (data.Site !='062r') & (data.Site !='068r') & (data.Site !='167l') & (data.SitePart == 'Eddy') & (data.Plane_Height == 'eddyabove25k')]   
...
subset1 = data[(data.Time_Series == 'short') & (data.Site !='m006r')& (data.Site !='033l') & (data.Site !='062r') & (data.Site !='068r') & 
               (data.Site !='167l') & (data.SitePart == 'Eddy') & (data.Plane_Height == 'eddyabove25k')]   
...
gc_early_plot_mean.plot(y = 'NormArea', yerr='std_error', ax = ax, label = 'Grand Canyon',linestyle='-',color='blue',marker='o' )
mc_early_plot_mean.plot(y = 'NormArea', yerr='std_error',ax = ax, label = 'Marble Canyon',linestyle='--',color='green',marker='x')
ax.set_xlim(pd.Timestamp('1990-01-01'), pd.Timestamp('2003-01-01'))
ax.set_ylabel('Normalized Sandbar Area [m$^2$/m$^2$]')
ax.set_xlabel('Date')
ax.set_title('Fluctuating Zone')
plt.tight_layout()
plt.savefig(r"C:\workspace\Time_Series\Output\sediment_deficit_HE_area.png",dpi=600)
...
gc_late_plot_mean.plot(y = 'NormArea', yerr='std_error', ax = ax1, label = 'Grand Canyon',linestyle='-',color='blue' ,marker='o')
mc_late_plot_mean.plot(y = 'NormArea', yerr='std_error',ax = ax1, label = 'Marble Canyon',linestyle='--',color='green',marker='x')
ax1.set_xlim(pd.Timestamp('2003-01-01'), pd.Timestamp('2016-01-01'))
ax1.set_ylabel('Normalized Sandbar Area [m$^2$/m$^2$]')
ax1.set_xlabel('Date')
ax1.set_title('Fluctuating Zone')
plt.tight_layout()
plt.savefig(r"C:\workspace\Time_Series\Output\sediment_enrichment_HE_area.png",dpi=600)
