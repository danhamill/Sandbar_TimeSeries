...
umc_early_vol = pd.pivot_table(umc_early_vol,values=['Early_Vol'], index=['TripDate'], aggfunc=np.average)
del table1
...
lmc_early_vol = pd.pivot_table(lmc_early_vol,values=['Early_Vol'], index=['TripDate'], aggfunc=np.average)
del table2
#Associate Early_Vol with early dates
...
with PdfPages(r'C:\workspace\Time_Series\Output\MC_Long_Term_average_percent_vol.pdf') as pdf:
