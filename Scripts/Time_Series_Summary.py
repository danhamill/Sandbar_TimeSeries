...
vol_change = df_15[['Site','Volume','Segment']]
vol_change['Volume'] = vol_change['Volume'] -df_90['Volume'].values
...
vol_change.groupby('long_trend').size().plot(ax=ax,kind='bar',rot=0,title='All Sites')
ax.set_xlabel('')
ax.set_ylabel('Number of Sites')

ax = fig.add_subplot(2,3,2)
vol_change.query('Segment == ["1_UMC","2_LMC"]').groupby('long_trend').size().plot(ax=ax,kind='bar',rot=0,title='Marble Canyon',ylim=(0,35))
ax.set_xlabel('')
ax.set_ylabel('Number of Sites')

ax = fig.add_subplot(2,3,3)
vol_change.query('Segment != ["1_UMC","2_LMC"]').groupby('long_trend').size().plot(ax=ax,kind='bar',rot=0,title='Grand Canyon',ylim=(0,35))
ax.set_xlabel('')
ax.set_ylabel('Number of Sites')
...
plt.savefig(r"C:\workspace\Time_Series\output\Long_Term_Monitoring_Sites_Summary.png", dpi=600)
plt.show()
