...
if __name__ =='__main__':
    ...
    for group1 in [g_1a,g_1b,g_1c,g_2,g_3,g_4][1:2]:
        ...
        def plot_metrics(subset,canyon):
            ...
            #Volume Plot
            tmp.plot(x='TripDate',y='Volume',yerr='Errors',ax=ax_0,label=r[n]+': ' +canyon, sharex=ax_2,capsize=2,zorder=10,marker=m[n],ms=m_size,c=colors[n],linestyle=ls[n])
            tmp_1.plot(x='TripDate',y='Volume',yerr=std_err_calc(subset,'Volume'), ax=ax_1,sharex=ax_2,capsize=2,zorder=10,marker=m[n],ms=m_size,c=colors[n],linestyle=ls[n],label=r[n]+': ' +canyon)
            tmp1.plot(x='TripDate',y='Norm_Vol', yerr=std_err_calc(subset,'Norm_Vol'),ax=ax_2,label=r[n]+': ' +canyon,marker=m[n],capsize=2,zorder=10,ms=m_size,c=colors[n],linestyle=ls[n])
            
            #write to excel
            #excel_group(writer,tmp,tmp_1,tmp1,r[n])
            #Area Plot
            tmp.plot(x='TripDate',y='Area_2D',ax=ax1_0,label=r[n]+': ' +canyon,sharex=ax1_2,zorder=10,marker=m[n],ms=m_size,c=colors[n],linestyle=ls[n])
            tmp_1.plot(x='TripDate',y='Area_2D',yerr=std_err_calc(subset,'Area_2D'), ax=ax1_1,label=r[n]+': ' +canyon,sharex=ax1_2,capsize=2,zorder=10,marker=m[n],ms=m_size,c=colors[n],linestyle=ls[n])
    
            tmp1.plot(x='TripDate',y='Norm_Area', yerr=std_err_calc(subset,'Norm_Area'),marker=m[n],ax=ax1_2,label=r[n]+': ' +canyon,capsize=2,zorder=10,ms=m_size,c=colors[n],linestyle=ls[n])
        
        plot_metrics(mc,'Marble Canyon')
        plot_metrics(gc,'Grand Canyon')
        n+=1
